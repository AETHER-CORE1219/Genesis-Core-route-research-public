# 発現経路研究 GitHub公開 README 日本語版 v1
...
## 最新: 外部要因・固定22面の頑健性研究 v7

2026年8月21日、固定development420の五つの社会機能について、年代、現実資料の測定形態、収集元を一つずつ加えるM0→M3解析と、要因別第二検証、固定22面のperson-package maxT、adversarial decoy、Western/Jyotish片側比較を完了しました。

- 五機能すべてがM3でも`+27.95`～`+39.08`ポイントの正方向でした（身体競技を含めると範囲`+27.95`～`+39.08`、公益`+33.60`、技術`+32.49`、身体`+32.14`）。
- 22面同時補正後pは`0.0001`～`0.0047`、同時片側95%下限は五機能すべて0を上回りました。
- 年代層、主要測定形態、収集元一つ抜き、311名・109名の観測機会調整でも全機能が正方向でした。
- これは固定development420と固定22面の範囲で、測定済み三要因と宣言した同時nullに耐える関連です。独立人物外再現、因果、515個別規則の確認ではありません。

最新入口は [社会的発現経路・外部要因頑健性研究 v7](independent_social_expression_v7/README_JA.md) です。匿名420名matrixからM0→M3、第二検証、9,999回maxT、7,634 decoy比較、system ablationを再計算できます。
...
- 候補は、Amatya候補における土星的な義務・持続の担い手から木星的な判断・正当化能力への方向付き依存、Dusthana carrier、身体的競争・遂行への接続です。
- 開発面では14名中6名が直接支持し、311名側4/10、109名側2/4で各基礎率より高い方向でした。
- 人物外面では、完全鎖のdecidable該当者7名中0名、非該当者223名中27名に競技者anchorがあり、層調整差は`-0.05274`、置換p=`1.0`、decoy p=`0.63`でした。
- Dusthana carrierを外した同一意味核は調整差`+0.03379`でしたが、置換p=`0.5276`で未確認です。
- よって厳密な競技者経路は人物外再現せず、終端は`INCONCLUSIVE_STRICT_ROUTE_WITH_DIRECTION_AND_LIMITS_REPORTED`です。これは占星術全体や他経路のnullではありません。

入口は [独立・社会的発現経路研究 v5](independent_social_expression_v5/README_JA.md) です。匿名313名ledger、99 decoy集約、決定的再現スクリプト、主張境界を公開しています。

## 以前の人物外検証: 独立・社会的発現経路研究 v4

2026年8月17日、v3とは別の機能・人物面・候補契約として、固定済みJyotish二枝familyと「集団を組織して実現する行為」の条件付き人物外確認を一回実施しました。

- 固定母集団1,495名のうち、独立人物ページ1,457名を一次解析としました。候補family該当59名、非該当1,398名です。
- 記録された本人行為を判定できた範囲の層別risk differenceは `+0.07101`、HC3片側95%上限は `+0.17129`、層内置換pは `0.1357`、decoy特異性pは `0.1313` でした。
